...
from memopt.util import measure_peak_memory
...
def benchmark_model(model_config, model_name, results_list):
    ...
    peak_mem_init_fp32 = measure_peak_memory(device)
    ...
    forward_start_events = [
        torch.cuda.Event(enable_timing=True) for _ in range(NUM_TIMING_ITERS)
    ]
    forward_end_events = [
        torch.cuda.Event(enable_timing=True) for _ in range(NUM_TIMING_ITERS)
    ]
    backward_start_events = [
        torch.cuda.Event(enable_timing=True) for _ in range(NUM_TIMING_ITERS)
    ]
    backward_end_events = [
        torch.cuda.Event(enable_timing=True) for _ in range(NUM_TIMING_ITERS)
    ...
    for step in range(NUM_TIMING_ITERS):
        ...
        torch.cuda.synchronize()
    peak_mem_fp32 = measure_peak_memory(device)

    # Calculate timings after all iterations complete
    for step in range(NUM_TIMING_ITERS):
        forward_times_fp32.append(
            forward_start_events[step].elapsed_time(forward_end_events[step]) / 1000.0
        )
        backward_times_fp32.append(
            backward_start_events[step].elapsed_time(backward_end_events[step]) / 1000.0
        )
        step_times_fp32.append(
            step_start_events[step].elapsed_time(step_end_events[step]) / 1000.0
        )
    ...
    print(
        f"  Forward:   {avg_forward_fp32:.4f} seconds ({avg_forward_fp32/total_fp32*100:.1f}%)"
    )
    print(
        f"  Backward:  {avg_backward_fp32:.4f} seconds ({avg_backward_fp32/total_fp32*100:.1f}%)"
    )
    print(
        f"  Optimizer: {avg_step_fp32:.4f} seconds ({avg_step_fp32/total_fp32*100:.1f}%)"
    )
    print(f"  Total:     {total_fp32:.4f} seconds")
    print(f"FP32 memory: init={peak_mem_init_fp32}, peak={peak_mem_fp32}")
    ...
    def benchmark_trainer(trainer, config_name, token_ids, targets, model_config):
        ...
        for step in range(NUM_TIMING_ITERS):
            forward_times.append(
                forward_start_events[step].elapsed_time(forward_end_events[step])
                / 1000.0
            )
            backward_times.append(
                backward_start_events[step].elapsed_time(backward_end_events[step])
                / 1000.0
            )
            step_times.append(
                step_start_events[step].elapsed_time(step_end_events[step]) / 1000.0
        ...
        print(
            f"  Backward:  {avg_backward:.4f} seconds ({avg_backward/total*100:.1f}%)"
        )
    ...
    peak_mem_init_autocast = measure_peak_memory(device)
    ...
    avg_forward_autocast, avg_backward_autocast, avg_step_autocast, total_autocast = (
        benchmark_trainer(
            trainer_autocast, "FP16 (autocast)", token_ids, targets, model_config
        )
    )
    peak_mem_autocast = measure_peak_memory(device)
    print(
        f"FP16 (autocast) memory: init={peak_mem_init_autocast}, peak={peak_mem_autocast}"
    )
    ...
    peak_mem_init_fp16 = measure_peak_memory(device)
    ...
    (
        avg_forward_no_master,
        avg_backward_no_master,
        avg_step_no_master,
        total_no_master,
    ) = benchmark_trainer(
        trainer_no_master, "FP16 (no master)", token_ids, targets, model_config
    )
    peak_mem_fp16 = measure_peak_memory(device)
    print(f"FP16 (no master) memory: init={peak_mem_init_fp16}, peak={peak_mem_fp16}")
    ...
    peak_mem_init_no_autocast = measure_peak_memory(device)
    ...
    peak_mem_no_autocast = measure_peak_memory(device)
    print(
        f"FP16 (default) memory: init={peak_mem_init_no_autocast}, "
        f"peak={peak_mem_no_autocast}"
    )
    ...
    print(
        f"Total time autocast:   {total_autocast:.4f} sec ({total_fp32/total_autocast:.2f}x)"
    )
    print(
        f"Total time no_master:  {total_no_master:.4f} sec ({total_fp32/total_no_master:.2f}x)"
    )
...
if __name__ == "__main__":
    ...
    csv_filename = os.path.join(
        output_dir, f"latency_mixprec_benchmark_{timestamp}.csv"
    )
