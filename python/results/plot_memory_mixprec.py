...
fig.suptitle(
    "Peak Memory Usage: FP32 vs Mixed Precision Configurations",
    fontsize=14,
    fontweight="bold",
)
...
bars1 = ax.bar(
    x - 1.5 * width, peak_fp32, width, label="FP32", color="#e74c3c", alpha=0.85
)
bars2 = ax.bar(
    x - 0.5 * width,
    peak_autocast,
    width,
    label="FP32-FP16 (autocast)",
    color="#3498db",
    alpha=0.85,
)
bars3 = ax.bar(
    ...
    peak_default,
    width,
    label="FP32-FP16 (no autocast)",
    color="#2ecc71",
    alpha=0.85,
)
bars4 = ax.bar(
    x + 0.5 * width, peak_no_master, width, label="FP16", color="#f39c12", alpha=0.85
)
...
for idx, row in df.iterrows():
    ...
    print(
        f"  Peak memory autocast:    {row['peak_mem_autocast_gb']:.3f} GB ({(1 - row['peak_mem_autocast_gb']/row['peak_mem_fp32_gb'])*100:.1f}% saved)"
    )
    print(
        f"  Peak memory no_master:   {row['peak_mem_no_master_gb']:.3f} GB ({(1 - row['peak_mem_no_master_gb']/row['peak_mem_fp32_gb'])*100:.1f}% saved)"
    )
    print(
        f"  Peak memory default:     {row['peak_mem_default_gb']:.3f} GB ({(1 - row['peak_mem_default_gb']/row['peak_mem_fp32_gb'])*100:.1f}% saved)"
    )
